...
- **Things Figma layers can't express are screenshotted, not approximated** (server path): CSS `filter` (except `drop-shadow()`, which becomes a shadow effect), rotation/scale/skew transforms, `clip-path`, masks, `mix-blend-mode`, `backdrop-filter`, vertical writing modes. The element comes back as an image layer named after the property. Whole sections are never flattened (limit ≈ 800×800 px / 4 media children). `text-shadow` maps onto a text effect. The bookmarklet path names such layers with a ⚠ instead.
