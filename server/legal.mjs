...
<p>An account is identified by an email address; sign-in is by one-time links sent to that address. License keys are shown once and stored only as a hash. You are responsible for keeping keys private; anything done with a key counts against your account. You can create and revoke keys at any time from your account page. One person per account on Free and Pro; a Team plan may be shared within one company.</p>

<h2>3. Plans, credits and billing</h2>
<p>A credit is one captured width of one page. Failed captures are not charged. Credits reset on the first day of each calendar month (UTC) and do not carry over. Current limits: Free ${P.free.credits} credits, Pro ${P.pro.credits}, Team ${P.team.credits.toLocaleString('en-US')}; widths per capture and parallel captures are shown on the pricing page.</p>
...
<p><b>Usage records.</b> For each capture: the URL you captured, the widths, the credits charged, how long it took, whether it succeeded, and a timestamp. This is what your monthly credits are metered from and what you see on your account page.</p>
