...
<p class="muted">No account yet? <a href="/buy/pro">Get Pro</a> · <a href="/buy/team">Get Team</a></p>`);
  ...
  accountPage({ account, quota, keys, token, newKey = null, notice = null }) {
    ...
    const rows = keys.map(k => `<tr><td><code style="display:inline;padding:2px 6px">${esc(k.prefix)}…</code></td><td>${esc(k.label || '')}</td><td>${fmt(k.created_at)}</td><td>${fmt(k.last_used_at)}</td><td>${k.revoked_at ? '<span class="muted">revoked</span>' : `<form method="post" action="/account/revoke" style="display:inline"><input type="hidden" name="token" value="${esc(token)}"><input type="hidden" name="prefix" value="${esc(k.prefix)}"><button style="font:13px system-ui;background:none;border:1px solid #ccc;border-radius:6px;padding:4px 8px;cursor:pointer">Revoke</button></form>`}</td></tr>`).join('');
    return page('Your account', `<style>table{border-collapse:collapse;width:100%;font-size:14px}td,th{text-align:left;padding:8px 6px;border-bottom:1px solid #eee}th{color:#666;font-weight:500}.bar{height:8px;background:#eee;border-radius:4px;overflow:hidden}.bar i{display:block;height:100%;background:#111}.row{display:flex;gap:10px;flex-wrap:wrap;align-items:center}</style>
...
<table><tr><th>Key</th><th>Label</th><th>Created</th><th>Last used</th><th></th></tr>${rows || '<tr><td colspan="5" class="muted">No keys yet</td></tr>'}</table>
